...
const AddUsers = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const selectedOffice = offices.find(officeItem => officeItem._id === office);
    const officeName = selectedOffice ? selectedOffice.office : "";
  
    axios.post('http://localhost:3001/add-user', {firstname, lastname, email, password, position, office: officeName})
      .then(res => {
          ...
          <div className="FormWrapper">
            <form action="" className="AddUserForm" onSubmit={handleSubmit}> 
              <div className="FormText">
                ...
                <p>Office Name:</p>
                <div className="input-new">
                  <select id="office" className="input-field" required value={office} onChange={(e) => setOffice(e.target.value)}>
                    <option value="" disabled>Select here</option>
                    {offices.map((officeItem, index) => (
                      <option key={index} value={officeItem._id}>{officeItem.office}</option>
                    ))}
                  </select>
                </div>
                ...
                <p>Position:</p>
                <div className="input-new">
                  <input type="text" id="position" required value={position}
